---
name: coop
description: Route explicit Agent Co-op launch or join requests to the canonical CLI. Use when the user invokes /coop, asks Claude to use Agent Co-op, joins a live Co-op, or when COOP_SESSION_ID already binds the harness. Never activate only because an ordinary task appears suitable for multiple agents.
---

# Agent Co-op — Claude launch entrypoint
...
`/coop` or asks to use Agent Co-op. The request must identify an existing item,
a new goal, or the whole board. Never infer launch authorization from task
suitability.

Route an unbound request as follows:

1. For `/coop <item-id>` or a named existing item, run
   `coop start --item <id>`.
2. For an explicit whole-board request, run `coop start --all`. Never
   substitute `--all` for a missing target.
3. For `/coop <goal text>` or an explicit new goal, run
...
4. For `/coop` without a target, run `coop tasks` and ask the user to select an
   item. Do not guess.
...
run, bind before any agent mutation:

```bash
eval "$(coop session start --as <claude|codex|grok> --export)"
```
